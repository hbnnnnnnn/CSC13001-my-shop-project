...
function loadConfig() {
  ...
  try {
    ...
    console.error('Error reading db.config.json, using fallback config:', error.message);
  }
...
const updateConfig = async (newConfig) => {
  ...
  try {
    ...
    testPool.end().catch(() => { });
    console.error('Failed to update DB configuration. Keeping the old connection pool.');
    throw new Error('Could not connect to database with new config: ' + err.message);
  }
};
